...
jsdom.env(
    ...
    function (err, window) {
        ...
        for (var a = 0; a < inputs.length; a++)
        {
            ...
            {
                addString = inputs[a].textContent.replace(/\s\s+/g, '') + " " + inputs[a].getElementsByTagName("input")[0].name.replace(/\s\s+/g, '');

                //output.push({key: inputs[a].textContent.replace(/\s/g, ''), value: addString});
                output[inputs[a].textContent.replace(/\s\s+/g, '')] = addString;


            }
            else if (inputs[a].getElementsByTagName("select")[0] != undefined) {

                addString = inputs[a].getElementsByTagName("td")[0].textContent.replace(/\s\s+/g, '') + " " + inputs[a].getElementsByTagName("select")[0].name.replace(/\s\s+/g, '');
